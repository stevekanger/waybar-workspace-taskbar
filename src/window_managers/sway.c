...
/**
 * Click handler to minimize the window. Sway has no true minimize,
 * so we move the window to the scratchpad as a pseudo-minimize.
 *
 * @param id The window id
 * @return Whether the command was successfully executed
 */
static gboolean window_minimize(const char *id) {
    return wm_click_execute("swaymsg \"[con_id=%s] move scratchpad\"", id);
}

/**
 * Click handler to maximize the window. Sway has no true maximize,
 * so we resize to fill the container.
 *
 * @param id The window id
 * @return Whether the command was successfully executed
 */
static gboolean window_maximize(const char *id) {
    return wm_click_execute("swaymsg \"[con_id=%s] fullscreen\"", id);
}
...
/**
 * Click handler to minimize-raise the window. Sway has no true minimize,
 * so we use fullscreen toggle as a proxy.
 *
 * @param id The window id
 * @return Whether the command was successfully executed
 */
